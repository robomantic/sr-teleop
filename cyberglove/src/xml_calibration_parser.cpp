...
/**
 * construction method: parses the given file and stores the calibration
 * in the vector vector<JointCalibration> jointsCalibrations.
 *
 ...
 */
void XmlCalibrationParser::init(const string &path_to_calibration)
{
  TiXmlDocument doc(path_to_calibration.c_str());
  bool loadOkay = doc.LoadFile();
  if (loadOkay)
  {
    ROS_DEBUG_STREAM("loading calibration " << path_to_calibration);
  ...
  {
    ROS_ERROR_STREAM("Failed to load file : " << path_to_calibration);
  }
}

/**
 * Parses the calibration file and retrieve the full calibration for
 * the cyberglove.
 *
 ...
 */
vector<XmlCalibrationParser::Calibration>
XmlCalibrationParser::parse_joint_attributes(TiXmlNode* pParent)
{
  vector<XmlCalibrationParser::Calibration> calibrations;
...
int XmlCalibrationParser::build_calibration_table()
{
  for (unsigned int index_calib = 0; index_calib < jointsCalibrations.size(); ++index_calib)
  {
    ...
    vector<float> lookup_table((int) lookup_offset * (int) lookup_precision);
    ...
    //joints_calibrations_map.insert(pair <string, vector<float> >(name, lookup_table));
  }
 ...
 */
float XmlCalibrationParser::compute_lookup_value(int index, vector<XmlCalibrationParser::Calibration> calib)
{
  ...
  for (unsigned int index_calib = 0;
       index_calib < calib.size() - 1;
       ++index_calib)
  {
...
float XmlCalibrationParser::get_calibration_value(float position, const string &joint_name)
{
...
vector<XmlCalibrationParser::JointCalibration> XmlCalibrationParser::getJointsCalibrations()
{
