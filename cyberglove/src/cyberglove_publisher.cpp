...
CyberglovePublisher::CyberglovePublisher() :
  n_tilde("~"),
  sampling_rate(0.0),
  publish_counter_max(0),
  publish_counter_index(0),
  path_to_glove("/dev/ttyS0"),
  publishing(true)
{
  string path_to_calibration;
  n_tilde.param("path_to_calibration", path_to_calibration, string("/etc/robot/calibration.d/cyberglove.cal"));
  ...
  // set path to glove
  n_tilde.param("path_to_glove", path_to_glove, string("/dev/ttyS0"));
...
void CyberglovePublisher::add_jointstate(float position, const string &joint_name)
{
