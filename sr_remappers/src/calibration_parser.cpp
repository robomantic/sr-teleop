...
const string CalibrationParser::default_path = "/etc/robot/mappings/default_mapping";
...
CalibrationParser::CalibrationParser(const string &path)
{
  init(path);
}

int CalibrationParser::init(const string &path)
{
  ...
  {
    ROS_ERROR_STREAM("Couldn't open the file " << path);
    return -1;
  }
  ...
  while (getline(calibration_file, line))
  {
    ...
    vector<double> double_line(splitted_string.size());
  ...
  ostringstream ss;
  ss << "mapping matrix, from glove to hand\n";
  for (size_t line = 0; line < calibration_matrix.size(); ++line)
  {
    for (size_t col = 0; col < calibration_matrix[0].size(); ++col)
    {
...
vector<double> CalibrationParser::get_remapped_vector(vector<double> input_vector)
{
  ...
  if (input_vector.size() != calibration_matrix.size())
  {
    ...
    return vector<double>(calibration_matrix[0].size());
  }

  vector<double> result(calibration_matrix[0].size());
  double tmp_value;
